...
private:
    ...
    string _PrepareTransferLogRecord(float Amount, clsBankClient DestinationClient, string UserName,
        string Seperator = "#//#")
    {
...
public:
   ...
   /* void Print()
    ...
    {
        vector<stTransferLogRecord> vTrnasferLogRecord;
        ...
        if (MyFile.is_open())
        {
            ...
            stTransferLogRecord TransverRecord;

            while (getline(MyFile, Line))
            {

                TransverRecord = _ConvertTransferLogLineToRecord(Line);

                vTrnasferLogRecord.push_back(TransverRecord);
            }

            MyFile.close();
        }

        return vTrnasferLogRecord;
